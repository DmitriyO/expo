...
public abstract class BaseNotificationsService extends JobIntentService {
  ...
  //                                EXPO NOTIFICATIONS
  private static final int JOB_ID = 397606684;
  ...
  public static void enqueueReceive(Context context, @NonNull RemoteMessage remoteMessage) {
    ...
    intent.putExtra(NOTIFICATION_IDENTIFIER_KEY, identifier);
    intent.putExtra(NOTIFICATION_REQUEST_KEY, new JSONObject(remoteMessage.getData()).toString());
    intent.putExtra(NOTIFICATION_TRIGGER_KEY, new FirebaseNotificationTrigger(remoteMessage));
    enqueueWork(context, intent);
  }
